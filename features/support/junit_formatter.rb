  ...
  module Formatter
    ...
    class Junit
      ...
      def after_test_case(test_case, result)
        ...
        scenario_designation = "@#{$size} #{scenario}#{test_case_name.name_suffix}"
      ...
      def start_feature(feature)
        ...
        @current_feature.instance_variable_set(:@name, remove_sizes_from(@current_feature.name))
        @failures = @errors = @tests = @skipped = 0
        @builder = Builder::XmlMarkup.new(:indent => 2)
        @time = 0
      end

      def remove_sizes_from(text)
        $sizes.each { |size|
          text = text.gsub(size,"")
        }
        text.lstrip
      ...
      def build_testcase(result, scenario_designation, output)
        ...
        $sizes.each{ |size|
          classname = classname.gsub(size,"")
        }
        classname = $size + "_" + classname
